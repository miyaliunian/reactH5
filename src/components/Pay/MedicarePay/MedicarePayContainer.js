...
import icon_ybzf from '@assets/images/Pay/ico_ybk_png.png'
...
class MedicarePayContainer extends Component {
      ...
      <div className={'medicarePay'} style={{ height: '100vh', width: '100%', position: 'fixed' }}>
        <SafeAreaView showBar={true} title={'医保支付'} isRight={false} handleBack={this.handleBack}>
          <div className={'payComponent_info'}>
            ...
            <div className={'payComponent_info_row'}>
              <span>账户支付</span>
              <span style={{ color: 'orange' }}>￥{(orderPayment.siPayAmt + orderPayment.pubPayAmt).toFixed(2)}</span>
            </div>
          </div>
            ...
            <div className={'payComponent_payBtn_row border-top '}>
              ...
              <div className={'payComponent_payBtn_row_right'}>
                ...
                <span
                  ...
                  账户金额：￥{orderPayment.prePayBalance.toFixed(2)}
                </span>
              </div>
              <img src={icon_pay_radio_sel} className={'payComponent_payMethod'} />
            </div>
          </div>
          <div className={'payComponent_bottomBtn'}>
            <span className={'payComponent_desc'}>
              待支付：￥
              {(orderPayment.siPayAmt + orderPayment.pubPayAmt).toFixed(2)}
            </span>
            <span className={'payComponent_btn'} onClick={() => this.isLocalOrSiPay()}>
              支 付
            </span>
          </div>
          {/*本地电子社保卡 输入密码*/}
  ...
  isLocalOrSiPay() {
    ...
    const {
      popUpActions: { showPopup },
      medicarePayActions: { loadSignable },
      location
    } = this.props
    ...
    if (tag) {
      ...
      if (tagData.actionType === '009') {
        pay(
          ...
          orderPayment,
          { ...this.props.history },
          data => this.popGoBack(data)
        )
      }
    } else {
      //本地
      console.log('调用本地方法')
      pay(e, tag, '', orderType, ObjEntity, reservationName, orderPayment, { ...this.props.history }, data =>
        this.popGoBack(data)
      )
    }
  }
  ...
  componentDidMount() {
    const {
      history,
      location,
      medicarePayActions: { loadVerifyInfo }
    } = this.props

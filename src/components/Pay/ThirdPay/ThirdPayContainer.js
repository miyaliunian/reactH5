...
import {
  getPayMethodEntity,
  actions as thirdPayActions
} from "@reduxs/modules/thirdPay";
            ...
            <div className={"payComponent_info_row"}>
              ...
              {!from ? (
                <span style={{ color: "orange" }}>
                  ￥{orderPayment.ownPayAmt.toFixed(2)}
                </span>
              ) : (
                <span style={{ color: "orange" }}>
                  ￥{orderPayment.totalPayAmt.toFixed(2)}
                </span>
              )}
            </div>
          </OrderTypeWrapper>
          <div
            style={{ height: "15px", backgroundColor: "rgb(230,230,230)" }}
          />
          <PayTypeList>
            <p
            ...
            <ul>
              {PayMethodEntityItems.map((item, index) => {
                ...
                return (
                  <li
                    className={"payComponent_payBtn_row border-bottom "}
                    key={index}
                  >
                    <div
                      onClick={() => this.choosePayMethod(index)}
                      className={"payComponent_payBtn_item"}
                    >
                      <img
                        src={iconName}
                        style={{
          ...
          <ButtonWrapper>
            {!from ? (
              <span className={"payComponent_desc"}>
                待支付：￥{orderPayment.ownPayAmt.toFixed(2)}
              </span>
            ) : (
              <span className={"payComponent_desc"}>
                待支付：￥{orderPayment.totalPayAmt.toFixed(2)}
              </span>
            )}
  ...
  componentDidMount() {
    ...
    if (this.props.history.action === "PUSH") {
      ...
      if (!from) {
        ...
        if (
          paymentMethod === 1 &&
          orderPayment.ownPayAmt === orderPayment.totalAmt &&
          ObjEntity.paymentStatus === 0
        ) {
          //空跑一遍医保支付
          reMedicarePayAndReLoadPayTypeItems(
            reservationCode,
            ObjEntity,
            orderPayment
          );
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(ThirdPayContainer)
);

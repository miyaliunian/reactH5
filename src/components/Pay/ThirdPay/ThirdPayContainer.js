...
import './style.less'
//图标
import icon_zfb from '@assets/images/Pay/ico_zfb_png.png' // 支付宝
import icon_wechat from '@assets/images/Pay/logo_wechat(07-12-09-01-07).png' // 微信
import icon_ylzf from '@assets/images/Pay/logo_unionpay.png' // 银联
import icon_gsyh from '@assets/images/Pay/工商银行.png' // 工商银行
import icon_pay_n from '@assets/images/Pay/icon_pay_radio_n.png' // 支付方式 正常状态
import icon_pay_radio_sel from '@assets/images/Pay/icon_pay_radio_sel.png' // 支付方式 选中状态
...
class ThirdPayContainer extends Component {
        ...
        <SafeAreaView showBar={true} title={'选择支付方式'} isRight={false} handleBack={this.handleBack}>
          <OrderTypeWrapper>
            ...
            <div className={'payComponent_info_row'}>
              ...
              {!from ? (
                <span style={{ color: 'orange' }}>￥{orderPayment.ownPayAmt.toFixed(2)}</span>
              ) : (
                <span style={{ color: 'orange' }}>￥{orderPayment.totalPayAmt.toFixed(2)}</span>
              )}
            </div>
          </OrderTypeWrapper>
          <div style={{ height: '15px', backgroundColor: 'rgb(230,230,230)' }} />
          <PayTypeList>
            <p style={{ height: '35px', lineHeight: '35px' }} className={'border-bottom'}>
              请选择支付方式
            </p>
            <ul>
                ...
                return (
                  <li className={'payComponent_payBtn_row border-bottom '} key={index}>
                    <div onClick={() => this.choosePayMethod(index)} className={'payComponent_payBtn_item'}>
                      ...
                      <img
                        src={index === this.state.selIndex ? icon_pay_radio_sel : icon_pay_n}
          ...
          <ButtonWrapper>
            ...
            <span className={'payComponent_btn'} onClick={() => this.Pay()}>
              支 付
            </span>
          </ButtonWrapper>
        </SafeAreaView>
  ...
  componentDidMount() {
     ...
     */
    const {
      thirdPayActions: { reMedicarePayAndReLoadPayTypeItems, loadPayTypeItems }
    } = this.props

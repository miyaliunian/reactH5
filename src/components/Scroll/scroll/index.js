      ...
      return (
        <div className="b-pullup-wrapper">
          <div className="after-trigger" style={{ lineHeight: '.32rem' }}>
            ...
            <span style={{ color: '#999999', fontSize: '13px'}}>{typeof pullUpLoad === 'object' ? pullUpLoad.txt.more : '加载中...'}</span>
          </div>
        </div>
      )
    }
      ...
      return (
        <div className="b-pullup-wrapper">
          <div className="before-trigger">
            <span style={{ color: '#999999',fontSize: '13px' }}>{typeof pullUpLoad === 'object' ? pullUpLoad.txt.nomore : '加载完成'}</span>
          </div>
        </div>
      )
  ...
  renderPullUpDown() {
    ...
    if (pullDownRefresh && !beforePullDown && !pulling) {
      return (
        <div className="b-pulldown-wrapper" style={pullDownStyle}>
          <div className="after-trigger">
            <div><span>{typeof this.options.pullDownRefresh === 'object' ? this.options.pullDownRefresh.txt.success : '刷新完成'}</span>
            </div>
          </div>
        </div>
      )

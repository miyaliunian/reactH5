    ...
    if (item.hosResponse.length > 0) {
      return (
        <div>
          ...
          <div className={'intelligentWaitingItem__bottom__defnotice'}>{IntelligentWaiting_Message.MESSAGE}</div>
        </div>
      )
    } else {
      return <div className={'intelligentWaitingItem__bottom__hosresponse'}>{item.hosResponse}</div>
    }
  }
}

        ...
        return (
            <div className={'registerForm'}>
                <div className={'registerForm__cell border-bottom'}>
                    ...
                    <input className="registerForm__cell__right__name" name="r_userName"
                           placeholder="请输入姓名" onChange={onChange}/>
                </div>
                <div className={'registerForm__cell border-bottom'}>
                    <img src={icon_id} alt='' className={'registerForm__middle__icon'}/>
                    <input className="registerForm__cell__right__name" name="r_userIdNumber"
                           placeholder="请输入有效身份证号" onChange={onChange}/>
                </div>
                ...
                <div className={'registerForm__cell border-bottom'}>
                    ...
                    <input className="registerForm__cell__right__name" name="r_cellphoneNumber"
                           placeholder="请输入手机号码" onChange={onChange}/>
                    <div className={'registerForm__security_code'} onClick={() => {
                        this.rowClick(1)
                    }}><span className={'registerForm__security_code_btn border'}>获取验证码</span></div>
                </div>
                <div className={'registerForm__cell border-bottom'}>
                    <img src={icon_security_code} alt='' className={'registerForm__middle__icon'}/>
                    <input className="registerForm__cell__right__name" name="r_securityCode"
                           placeholder="请输入手机验证码" onChange={onChange}/>
                </div>
                <div className={'registerForm__cell border-bottom'}>
                    <img src={icon_password} alt='' className={'registerForm__middle__icon'}/>
                    <input type="password" className="registerForm__cell__right__name" name="r_password"
                           placeholder="请输入登录密码(6-15位字母+数字)" onChange={onChange}/>
                </div>
                <div className={'registerForm__cell border-bottom'}>
                    <img src={icon_password} alt='' className={'registerForm__middle__icon'}/>
                    <input type="password" className="registerForm__cell__right__name" name="r_passwordRepeat"
                           placeholder="再次输入登录密码" onChange={onChange}/>
                </div>
                <div className={'registerForm__btnContainer'}>
                    <button className={'registerForm__btn'} onClick={() => this.rowClick(2)}>{'注册'}</button>
                </div>
                <div className={'info'}>注册意味着表示您已经阅读并同意<a href="https://www.ntyibao.com/t-core/license/license.htm"><span>用户协议>></span></a> </div>
            </div>
        )
    }
                ...
                if(registerActions.checkCellphoneNumber()){
                    ...
                    const showAlert = () => {
                        const alertInstance = alert('获取短信验证码', '我们将发送短信验证码到这个号码: '+r_cellphoneNumber, [
                            ...
                            { text: '确定', onPress: () => registerActions.sendSmsMessage() },
                        ]);
                        setTimeout(() => {

...
class AdvanceSettlementContainer extends Component {
  ...
  render() {
    ...
    return (
      <div id={'AdvanceSettlementContainer'} style={{ height: '100vh', width: '100%', position: 'fixed' }}>
        ...
        <LoadingMask />
        <SlideDownSnackBar
          desc={'医保账户或个人自费费用尚未完成支付，系统将在20分钟后自动进行退费'}
          ...
          <div className={'infoRow_item_right'}>
            ...
            {person ? (
              ...
              <span
                ...
                {typeof postPayBalance !== 'undefined' ? postPayBalance.toFixed(2) : ''}
              </span>
            )}
          </div>
        </InfoRow>
        ...
        <InfoRow showBorder={true}>
          ...
          <div className={'infoRow_item_right'}>
            ...
            <span
              ...
              {typeof prePayBalance !== 'undefined' ? prePayBalance.toFixed(2) : ''}
            </span>
          </div>
        </InfoRow>
        <InfoRow showBorder={true}>
  ...
  componentDidMount() {
    ...
    const {
      history,
      advanceSettlementActions: { loadPersonAndAdvanceSettleInfo, setAdvanceSettleInfoANdLoadPerson }
    } = this.props
    if (history.action === 'PUSH') {

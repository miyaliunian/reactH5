...
class AdvanceSettlementContainer extends Component {
        ...
        >
          <PayStatusContent show={paymentStatus === 1 ? true : false}>
            <img
              src={icon_pay}
              alt=""
              className={"hospitalsItem__middle__icon"}
            />
          ...
          <PerContent>
            <img
              src={ico_user}
              className={"bindCard__icon"}
              width={"35px"}
              height={"35px"}
            />
            <span style={{ marginLeft: "10px", fontSize: "20px" }}>
              {reservationEntity.patientName}
            </span>
          </PerContent>
          <SettleInfoContent>
          ...
          <Separation />
          {paymentStatus === 1
            ? this.renderSettleInfo1()
            : this.renderSettleInfo0()}
          {this.renderBtnTitle()}
        </SafeAreaView>
        <LoadingMask />
        <SlideDownSnackBar
          desc={
            "医保账户或个人自费费用尚未完成支付，系统将在20分钟后自动进行退费"
          }
      ...
      <PayInfoContent>
        <InfoRow showBorder={true}>
          ...
          <div className={"infoRow_item_middle"}>
            <span>
              ￥{typeof pubPayAmt !== "undefined" ? pubPayAmt.toFixed(2) : ""}
            </span>
            <span style={{ marginLeft: "5px", color: "#0084ff" }}>
              [已支付]
            </span>
          </div>
          <div className={"infoRow_item_right"}></div>
        </InfoRow>
        <InfoRow showBorder={true}>
          <span className={"infoRow_item_title"}>账户支出</span>
          <div className={"infoRow_item_middle"}>
            <span>
              ￥{typeof siPayAmt !== "undefined" ? siPayAmt.toFixed(2) : ""}
            </span>
            <span style={{ marginLeft: "5px", color: "#0084ff" }}>
              [已支付]
            </span>
          ...
          <div className={"infoRow_item_right"}>
            ...
            {person ? (
              ...
              >
                {parseInt(person.account).toFixed(2)}
              </span>
            ) : (
              <span
                style={{
              ...
              >
                {typeof postPayBalance !== "undefined"
                  ? postPayBalance.toFixed(2)
                  : ""}
              </span>
            )}
          </div>
        ...
        <InfoRow showBorder={true}>
          ...
          <div className={"infoRow_item_middle"}>
            <span>
              ￥{typeof ownPayAmt !== "undefined" ? ownPayAmt.toFixed(2) : ""}
            </span>
          </div>
          <div className={"infoRow_item_right"}></div>
        </InfoRow>
  ...
  renderSettleInfo0() {
    ...
    let show = false;
    if (
      typeof ownPayAmt !== "undefined" &&
      typeof totalAmt !== "undefined" &&
      ownPayAmt === totalAmt
    ) {
      show = true;
    }
      ...
      <PayInfoContent>
        ...
        <InfoRow showBorder={true}>
          ...
          <div className={"infoRow_item_right"}>
            ...
            >
              {typeof prePayBalance !== "undefined"
                ? prePayBalance.toFixed(2)
                : ""}
            </span>
          </div>
        </InfoRow>
  ...
  renderBtnTitle() {
    ...
    return (
      <BtnContent height={20}>
        <ButtonWrapper
          txt={titleStr}
          onSubmit={() => this.navPage(targetUrl)}
          disabled={btnDisable}
        />
      </BtnContent>
    );
  ...
  componentWillMount() {
    ...
    if (history.action === "PUSH") {
      ...
      this.props.advanceSettlementActions.setPaymentStatus(
        reservationEntity.paymentStatus
      );
    }
  }
  ...
  componentDidMount() {
    ...
    const {
      history,
      advanceSettlementActions: {
        loadPersonAndAdvanceSettleInfo,
        setAdvanceSettleInfoANdLoadPerson
      }
    } = this.props;
    if (history.action === "PUSH") {
      ...
      if (reservationEntity.paymentStatus === 0) {
        ...
        loadPersonAndAdvanceSettleInfo(
          reservationCode,
          reservationEntity.unifiedOrderId,
          reservationEntity.patientId
        );
      } else {
        //医保已经支付
        setAdvanceSettleInfoANdLoadPerson(
...
const mapDispatchToProps = dispatch => {
  return {
    advanceSettlementActions: bindActionCreators(
      advanceSettlementActions,
      dispatch
    )
  };

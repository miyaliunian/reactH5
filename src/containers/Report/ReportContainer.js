...
import ReportItem
    from "@containers/Report/components/ReportItem/ReportItem";
...
class ReportContainer extends Component {
    ...
    render() {
        const {fetchingStatus, bindCardList, hospitalizationSel, reportList} = this.props
        return (
            <div className={'report'}>
                <SafeAreaView showBar={true} title={'报告查询'} isRight={false} handleBack={this.handleBack}>
                    {/*-------------------------选择成员信息*/}
                    <BindCardItem data={bindCardList} isRefresh={this.refresh}
                                  callBack={(data) => this.refreshCallBack(data)}/>
                    {/*-------------------------选择医院*/}
                    <div className={'report_hospitalization border-bottom'} onClick={() => {
                        this.setState({openModalHospitalization: true})
                    }}>
                        <span className={'report_hospitalization_text_Style'}>就诊医院</span>
    ...
    refreshCallBack(data) {
        ...
        this.props.reportActions.refreshReportListByOpenType(hospitalizationSel, data)
    }

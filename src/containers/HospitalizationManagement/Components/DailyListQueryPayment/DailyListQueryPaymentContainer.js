...
class DailyListQueryPaymentContainer extends Component {
  ...
  render() {
    ...
    return (
      <div className={"dailyListQueryPayment"}>
        ...
        >
          <DynamicTabs
            queryTitle={this.state.queryStatus}
            pre={() => this.pre()}
            nex={() => this.nex()}
          />
  ...
  componentDidMount() {
    ...
    this.props.dailyListQueryPaymentActions.initDailyQueryList(
      "inPrePay",
      hosId,
      inHosNo,
      startDate,
  ...
  pre() {
    ...
    let startDate = `${getStartDate(this.state.timeStep - 24 * 60 * 60 * 1000) +
      "000000"}`;
    let endDate = `${getStartDate(this.state.timeStep - 24 * 60 * 60 * 1000) +
      "235959"}`;
...
const mapDispatchToProps = dispatch => {
  return {
    dailyListQueryPaymentActions: bindActionCreators(
      dailyListQueryPaymentActions,
      dispatch
    )
  };
};
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(DailyListQueryPaymentContainer);

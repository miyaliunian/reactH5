...
const actionTypes = {
  ...
  CHOOSE_CATEGORY_HOS_SUCCESS:
    "CHOOSE_CATEGORY_HOS/CHOOSE_CATEGORY_HOS_SUCCESS",
  RESERVATION_HOSPITALS_SUCCESS:
    "CHOOSE_CATEGORY_HOS/RESERVATION_HOSPITALS_SUCCESS",
...
export const actions = {
  ...
  initCategaryHospitalList: (type, perObj, pageNu) => {
    return (dispatch, getstate) => {
      ...
      Axios.all([
        getAllHospitalList(
          url.API_QUERY_ALL_HOSPASTIENT(cityID, type, pageNu),
          dispatch
        ),
        getReserHospitalList(
          url.API_GET_REGED_LIST_BY_OPEN_TYPE(type, perObj.id),
          dispatch
        )
      ]).then(
  ...
  loadMoreAction: type => {
    return (dispatch, getstate) => {
      if (
        getstate().categoryHospList.pageNo <
        getstate().categoryHospList.pageCount
      ) {
...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case actionTypes.PULLUP_MORE_HOS:
      return {
        ...state,
        hospitalizationAll: state.hospitalizationAll.concat(

...
const actionTypes = {
  //住院服务
  FETCH_HOSPITALIZATION_REQUEST:
    "HOSPITALIZATION_MANAGEMENT/FETCH_HOSPITALIZATION_REQUEST",
  FETCH_HOSPITALIZATION_SUCCESS:
    "HOSPITALIZATION_MANAGEMENT/FETCH_HOSPITAL_SUCCESS",
  FETCH_HOSPITALIZATION_FAILURE:
    "HOSPITALIZATION_MANAGEMENT/FETCH_HOSPITALIZATION_FAILURE",
      ...
      return post(targetURL)
        .then(data => {
          if (data.infocode && data.infocode === 1) {
            ...
            let queryUrl = url.API_QUERY_INHOSPASTIENT(
              type,
              hosDef.id,
              perObj.id
            );

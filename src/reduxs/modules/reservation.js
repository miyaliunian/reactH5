...
const actionTypes = {
    ...
    // 医疗类别
    SET_MEDICAL_TYPE: 'RESERVATION/SET_MEDICAL_TYPE',
...
export const actions = {
    ...
    loadBindCardAndMedicalTypeList: () => {
        return (dispatch, getstate) => {
            ...
            return post(targetURL)
                .then(data => {
                        debugger
                        let defBind = data.data.filter(item => item.def === true)[0]
                        //家庭成员
                        dispatch(fetchBindCardSuccess(defBind))
                        //Switch组件 是否选中 是否可以切换状态
                        if (getstate().reservation.payType.showSwitch) {
                            if (defBind.sitype && defBind.auth) {
                                dispatch(
                                    setSwitchInfo({
                                        showSwitch: true,
                ...
                .then(res => {
                    //获取医疗类别(家庭成员)
                    debugger
                    const targetURL = URL.API_REGISTER_MEDICAL_TYPE(res.id)
                    return post(targetURL)
                        .then(data => {
        ...
        return (dispatch, getstate) => {
            ...
            return post(targetURL)
                .then(
                    data => {
                        //使用家庭成员的SiTypeCode获取医疗类别
                        dispatch(fetchMedicalTypeSuccess(data.data[0]))
                    }
                )
        }
    },
    ...
    //设置医疗类别
    setMedicalType: data => ({
        type: actionTypes.SET_MEDICAL_TYPE,
        data
    }),
...
const reducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case actionTypes.SET_MEDICAL_TYPE:
            return {
                ...state,
                isFetching: false,
                medicalTypeData: action.data
